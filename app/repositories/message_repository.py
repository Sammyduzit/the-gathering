...
class IMessageRepository(BaseRepository[Message]):
    ...
    @abstractmethod
    def create_room_message(self, sender_id: int, room_id: int, content: str) -> Message:
        """Create a room-wide message."""
        pass

    @abstractmethod
    def create_conversation_message(self, sender_id: int, conversation_id: int, content: str) -> Message:
        """Create a conversation message (private/group)."""
        pass

    @abstractmethod
    def get_room_messages(self, room_id: int, page: int = 1, page_size: int = 50) -> Tuple[List[Message], int]:
        """Get room messages with pagination."""
        pass

    @abstractmethod
    def get_conversation_messages(self, conversation_id: int, page: int = 1, page_size: int = 50) -> Tuple[List[Message], int]:
...
class MessageRepository(IMessageRepository):
    ...
    def get_room_messages(self, room_id: int, page: int = 1, page_size: int = 50) -> Tuple[List[Message], int]:
        ...
        messages_query = select(Message, User.username).join(
        ...
        ).order_by(desc(Message.sent_at)).offset(offset).limit(page_size)
    ...
    def get_user_messages(self, user_id: int, limit: int = 50) -> List[Message]:
        ...
        ).order_by(desc(Message.sent_at)).limit(limit)
    ...
    def get_latest_room_messages(self, room_id: int, limit: int = 10) -> List[Message]:
        """Get latest messages from a room."""
        query = select(Message, User.username).join(
            User, Message.sender_id == User.id
        ).where(
            and_(
                Message.room_id == room_id,
